...
for (const url of [
  ...
  'http://localhost:3001/progetti/esempio-scheda',
]) {
  ...
  const r = await p.evaluate(() => {
    ...
    document.querySelectorAll('body *').forEach((el) => {
      ...
      // `data-decorativo`: testo che non porta informazione, sempre accompagnato
      // da `aria-hidden`. WCAG 1.4.3 esclude esplicitamente il testo puramente
      // decorativo, e il numerone tono su tono di Storey è quello. L'attributo
      // si dichiara a mano proprio perche' l'eccezione resti visibile con un
...
for (const g of guasti)
  console.log(
    `  ${g.c}:1 (serve ${g.soglia}) ${g.size}px w${g.weight} ${g.fg} su ${g.bg} — .${g.cls} «${g.testo}» ${g.url}`,
  )
