...
for (const [n,u] of [['A','http://localhost:3001/'],['B','http://localhost:3001/opzione-b'],['S','http://localhost:3001/servizi/strutture']]) {
  ...
  const r = await p.evaluate(() => {
    ...
    return {
      ...
      primaDiLoad: Math.round((somma(x => x.responseEnd <= nav.loadEventEnd) + (nav.transferSize || 0)) / 1024),
  ...
  const dopo = await p.evaluate(() => {
    ...
    return { videoKB: Math.round(v.reduce((a,x)=>a+(x.transferSize||0),0)/1024), videoStart: v.map(x=>Math.round(x.startTime)), tutte: res.length + 1 }
  })
